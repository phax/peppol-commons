...
public enum EPeppolNetwork implements IPeppolNetwork
{
   ...
   * Peppol Test Network
   */
  TEST ("test", "Peppol Test Network", "https://test-directory.peppol.eu", ESML.PEPPOL_TEST),
  /**
   * Peppol Production Network
   */
  PRODUCTION ("prod", "Peppol Production Network", "https://directory.peppol.eu", ESML.PEPPOL_PRODUCTION);

  // This is a temporary fallback that will be remo
  ...
  public static EPeppolNetwork getFromSMLInfoOrNull (@Nullable final ISMLInfo aSMLInfo)
  {
    ...
    if (aSMLInfo != null)
      for (final var e : EPeppolNetwork.values ())
      {
        ...
        if (e.m_eSmlOp.getID ().equals (aSMLInfo.getID ()))
          return e;
      }
    return null;
  }
