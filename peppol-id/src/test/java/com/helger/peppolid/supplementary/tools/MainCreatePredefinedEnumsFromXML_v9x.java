...
public final class MainCreatePredefinedEnumsFromXML_v9x
{
  ...
  @NonNull
  private static ECodeListItemState _getState (@NonNull final PCLStateType eState)
  {
    return switch (eState)
    {
      case ACTIVE -> ECodeListItemState.ACTIVE;
      case DEPRECATION_SCHEDULED, DEPRECATED -> ECodeListItemState.DEPRECATED;
      case REMOVED -> ECodeListItemState.REMOVED;
  ...
  private static void _handleDocumentTypes (final Document aDocumentSheet)
  {
    ...
    try
    {
      ...
      for (final PCLDocumentTypeType aRow : aList.getDocumentType ())
      {
        ...
        final ECodeListItemState eState = _getState (aRow.getState ());
        ...
        final IPeppolGenericDocumentTypeIdentifierParts aDocIDParts = PeppolGenericDocumentTypeIdentifierParts.extractFromString (sValue);
        IPeppolDocumentTypeIdentifierParts aXMLDocIDParts;
        if (PeppolDocumentTypeIdentifierParts.isSyntaxSpecificIDLookingLikeXML (aDocIDParts.getSyntaxSpecificID ()))
          aXMLDocIDParts = PeppolDocumentTypeIdentifierParts.extract (aDocIDParts);
        else
          aXMLDocIDParts = null;
        ...
        jEnumConst.arg (JExpr.lit (sScheme));
        {
          ...
          if (aXMLDocIDParts != null)
          {
            aNew = JExpr._new (CM.ref (PeppolDocumentTypeIdentifierParts.class))
                        .arg (aXMLDocIDParts.getRootNS ())
                        .arg (aXMLDocIDParts.getLocalName ())
                        .arg (aXMLDocIDParts.getCustomizationID ())
                        .arg (aXMLDocIDParts.getVersion ());
          }
          else
          {
            aNew = JExpr._new (CM.ref (PeppolGenericDocumentTypeIdentifierParts.class))
                        .arg (aDocIDParts.getSyntaxSpecificID ())
        ...
        jEnumConst.arg (CM.ref (ECodeListItemState.class).enumConstantRef (eState));
      ...
      final JFieldVar fParts = jEnum.field (JMod.PRIVATE | JMod.FINAL,
                                            IPeppolGenericDocumentTypeIdentifierParts.class,
                                            "m_aParts");
      ...
      final JFieldVar fState = jEnum.field (JMod.PRIVATE | JMod.FINAL, ECodeListItemState.class, "m_eState");
      ...
      final JVar jState = jCtor.param (JMod.FINAL, ECodeListItemState.class, "eState");
      ...
      m = jEnum.method (JMod.PUBLIC, IPeppolDocumentTypeIdentifierParts.class, "getXMLParts");
      m.annotate (Nullable.class);
      m.body ()
       ._return (JExpr.cond (fParts._instanceof (CM.ref (IPeppolDocumentTypeIdentifierParts.class)),
                             JExpr.cast (CM.ref (IPeppolDocumentTypeIdentifierParts.class), fParts),
                             JExpr._null ()));
      ...
      m = jEnum.method (JMod.PUBLIC, ECodeListItemState.class, "getState");
      m.annotate (NonNull.class);
  ...
  public static void main (final String [] args) throws Exception
  {
    for (final CodeListFile aCLF : new CodeListFile [] { new CodeListFile ("Document types",
                                                                           MainCreatePredefinedEnumsFromXML_v9x::_handleDocumentTypes),
                                                         new CodeListFile ("Participant identifier schemes",
                                                                           MainCreatePredefinedEnumsFromXML_v9x::_handleParticipantIdentifierSchemes),
